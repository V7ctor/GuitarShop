<?php if(!class_exists('Rain\Tpl')){exit;}?><form method="post" action="https://pagseguro.uol.com.br/v2/checkout/payment.html">  
        ...
        <input name="itemDescription<?php echo htmlspecialchars( $counter1+1, ENT_COMPAT, 'UTF-8', FALSE ); ?>" type="hidden" value="<?php echo htmlspecialchars( $value1["nameproduct"], ENT_COMPAT, 'UTF-8', FALSE ); ?>">  
        ...
        <input name="shippingAddressPostalCode" type="hidden" value="<?php echo htmlspecialchars( $order["zipcode"], ENT_COMPAT, 'UTF-8', FALSE ); ?>">  
        <input name="shippingAddressStreet" type="hidden" value="<?php echo utf8_encode($order["address"]); ?>">  
        <input name="shippingAddressNumber" type="hidden" value="<?php echo utf8_encode($order["number"]); ?>">  
        <input name="shippingAddressComplement" type="hidden" value="<?php echo utf8_encode($order["complement"]); ?>">  
        <input name="shippingAddressDistrict" type="hidden" value="<?php echo utf8_encode($order["district"]); ?>">  
        <input name="shippingAddressCity" type="hidden" value="<?php echo utf8_encode($order["city"]); ?>">
        <input name="shippingAddressState" type="hidden" value="<?php echo utf8_encode($order["state"]); ?>"> 
        <input name="shippingAddressCountry" type="hidden" value="<?php echo utf8_encode($order["country"]); ?>">  
  
        <!-- Dados do comprador (opcionais) -->  
        <input name="senderName" type="hidden" value="<?php echo utf8_encode($order["nameperson"]); ?>">  
        ...
        <input name="senderEmail" type="hidden" value="<?php echo htmlspecialchars( $order["emailuser"], ENT_COMPAT, 'UTF-8', FALSE ); ?>">  
